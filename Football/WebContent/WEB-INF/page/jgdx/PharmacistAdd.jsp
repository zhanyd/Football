...
$(document).ready(function () {
    $("#PharmacistAddDRUGSTORENAME").autocomplete(<%= session.getAttribute("jsonresult")%>, DrugAutocompleteSeting)
    .result(function (event, data, formatted) { //回调
        $('#PharmacistAddDRUGSTORENAME')[0].value = data.name;
    });
});
	function Main() {
	            ...
	            if (selectvalue.length > 0)
	            	{
			            ...
			            $("#PharmacistAddDRUGSTORENAME").val(str[1]);
	            	}
...
<body>
	<form id="form1" name="form1" method="post"
		action="addPharmacistManage.action" onsubmit="return validate();">
		<table border="0" cellspacing="0" cellpadding="0" id="tablewi4">
			...
			<tr>
				...
				<td>
					<s:textfield type="text" name="pharmacist.DRUGSTORENAME"  id="PharmacistAddDRUGSTORENAME" ck="rq" />
		...
		<input type="submit" name="submit" id="submit" value="提交" /> <input
			type="button" name="" value="返回" onclick="Main();" />
	</form>
