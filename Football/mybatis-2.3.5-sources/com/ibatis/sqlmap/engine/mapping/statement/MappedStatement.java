...
import com.wildwolf.action.SysConstant;
...
public class MappedStatement {
  ...
  public Object executeQueryForObject(StatementScope statementScope, Transaction trans, Object parameterObject, Object resultObject)
    ...
    try {
      ...
      executeQueryWithCallbackWithPage(1,statementScope, trans.getConnection(), parameterObject, resultObject, rowHandler, SqlExecutor.NO_SKIPPED_RESULTS, SqlExecutor.NO_MAXIMUM_RESULTS);
  ...
  public List executeQueryForList(StatementScope statementScope, Transaction trans, Object parameterObject, int skipResults, int maxResults)
    ...
    try {
      DefaultRowHandler rowHandler = new DefaultRowHandler();
      executeQueryWithCallbackWithPage(2,statementScope, trans.getConnection(), parameterObject, null, rowHandler, skipResults, maxResults);
  ...
  protected void executeQueryWithCallbackWithPage(int type,StatementScope statementScope, Connection conn, Object parameterObject, Object resultObject, RowHandler rowHandler, int skipResults, int maxResults)
      throws SQLException {
    ErrorContext errorContext = statementScope.getErrorContext();
    errorContext.setActivity("preparing the mapped statement for execution");
    errorContext.setObjectId(this.getId());
    ...
    try {
      ...
      //for page, added by wildwolf 2012-12-30 17:01
      sqlString = sqlString.replace(" ", " ");//去掉 中文空格
      if (parameterObject instanceof Page) {//if page
    	  Page p = (Page) parameterObject;
    	  if (p.isPage()){
    	  if (type ==1) {//return row amount
    		  
    		  String lowerCaseSql = sqlString.toLowerCase();
    			int groupIndex = lowerCaseSql.indexOf(" group ");
    			if (groupIndex == -1) {
    				  //optimize performance    	  			
    	  			int index = lowerCaseSql.indexOf("from");
    	  			String fromStatment = sqlString.substring(index);
    	  			sqlString =  "select count(*) " + fromStatment;
    			}
    			else {//used for "group by" and aggregation function
    	    		//bad performance   
    	    		  String preAppend = "select count(*) from (";
    	  			String postAppend = ")" + "  t_" + System.currentTimeMillis();
...
//  			AutoResultMap resultMap2 = new AutoResultMap(resultMap.getDelegate(), false);
//  			resultMap2.setId(sqlId+ "-AutoResultMap");
//  			resultMap2.setResultClass(Class.forName("java.lang.Long"));
//  			resultMap = resultMap2;
//  			
  			
//			resultMap2.setId(sqlId + "-AutoResultMap");
    	  }
    	  else if (type == 2) {//return row list
    		  /** ************ for page ********************** */
...
//  			String pageEndRowNo = String.valueOf(rownumMap.get("pageEndRowNo"));
  			...
  			String pageStartRowNo = String.valueOf(page.getBeginRow());
  			String pageEndRowNo = String.valueOf(page.getEndRow());
  			...
  			if(System.getProperty(SysConstant.DB_TYPE) == null)
  			{
  				System.setProperty(SysConstant.DB_TYPE, SysConstant.MYSQL_DB);
  			}
  			if (System.getProperty(SysConstant.DB_TYPE).equals(SysConstant.ORACLE_DB)) {  				
  			...
  			else if (System.getProperty(SysConstant.DB_TYPE).equals(SysConstant.MYSQL_DB)) {
  				sqlString = new StringBuffer().append(oldSql).append(" limit ").append(page.getBeginRow()-1).append(",").append(page.getEndRow()-page.getBeginRow()+1).toString();
  			}
